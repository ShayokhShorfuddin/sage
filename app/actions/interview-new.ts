...
async function handleMessageSubmission(formData: FormData) {
  ...
  if (!response.success) {
    console.error("Failed to send message to Gemini");
    return;
  }
...
async function createInterviewRouteAction(formData: FormData) {
  const interviewerName = formData.get("interviewer-card") as string;
  ...
  //   Redirect user to the newly created interview route
...
async function SendMessageToGeminiAction({
  ...
  // TODO: First, we need to send the text to Gemini and get the response. If we succeed, we will save both user message and Gemini response to chat history.

  // TODO: Get the interviewer name and chat history from interview data
  ...
  // TODO: Generate a response from Gemini here and save it to chat history.
